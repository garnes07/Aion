...
namespace Aion.Areas.WFM.ViewModels.Deployment
{
    public class DeploymentDetailECVm : BaseVm
    {
        ...
        private vw_DashboardData_v2_Top100 _StoreData;
        public vw_DashboardData_v2_Top100 StoreData => _StoreData ?? (_StoreData = WeekData.First());
        ...
        public string GetAdvice(decimal? toMove, int day)
        {
            decimal _toMove = (decimal)toMove;
            decimal rounded = Math.Round(_toMove * 4) / 4;

            if (day == 1)
                rounded -= DailyData.SundayCredit;
            else if (day == 7)
                rounded -= DailyData.SaturdayCredit;
