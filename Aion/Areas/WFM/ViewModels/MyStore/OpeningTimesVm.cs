...
namespace Aion.Areas.WFM.ViewModels.MyStore
{
    public class OpeningTimesVm : BaseVm
    {
        ...
        public List<StoreOpeningTime> PendingTimes => Collection.Where(x => x.Status.StartsWith("Pending")).OrderBy(x => x.EffectiveDate).ToList();
        public List<StoreOpeningTime> PeakTimes => Collection.Where(x => x.Status.StartsWith("Peak")).OrderBy(x => x.EffectiveDate).ToList();
    }
}